...
export default function Home() {
  ...
  return (
    <div className="w-full">
      <section className="relative h-[calc(100dvh-7rem)] md:h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-background">
        <Hero3D />

        <div ref={textRef} className="relative z-10 text-center pointer-events-auto opacity-0 px-6 mt-[-8vh] md:mt-[-10vh] w-full max-w-5xl">
          <h1 className="font-serif text-5xl sm:text-7xl md:text-[8.5rem] font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-primary to-secondary">
            CONNECT
          </h1>
          <div className="font-sans text-lg sm:text-2xl md:text-3xl text-accent tracking-[0.4em] sm:tracking-[0.5em] uppercase font-medium -mt-1 sm:-mt-2 md:-mt-4">
            Lab
          </div>
          <p className="font-sans text-sm sm:text-lg md:text-xl text-secondary max-w-3xl mx-auto mt-8 md:mt-12 leading-relaxed font-light">
          ...
          <Link to="/research">
            <button className="mt-10 md:mt-16 px-8 sm:px-12 py-4 sm:py-5 bg-fg text-background text-sm sm:text-base font-medium tracking-widest uppercase hover:bg-accent hover:-translate-y-1 hover:shadow-lg transition-all duration-300 rounded-sm">
              Explore Research
            </button>
          </Link>
        </div>
