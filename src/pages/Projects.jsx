...
const ProjectList = ({ title, projects, isHighlight = false }) => (
  <section className="mb-24">
    <h2 className={`font-serif text-3xl font-bold mb-10 pb-4 border-b ${isHighlight ? 'text-accent border-accent/30' : 'text-primary border-white/20'}`}>
      {title}
    </h2>
    <div className="flex flex-col space-y-8">
      {projects.map((project, idx) => (
        <article key={idx} className="flex flex-col md:flex-row md:items-start gap-2 md:gap-8 group">
          <div className="text-white/40 font-mono text-sm shrink-0 md:w-40 pt-1 group-hover:text-accent transition-colors">
            {project.period}
          </div>
          <div className="flex-grow">
            <h3 className="text-lg text-primary font-medium leading-relaxed group-hover:text-white transition-colors mb-1">
              {project.title}
            </h3>
            {project.role && (
